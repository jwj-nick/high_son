...
var VG = (function () {
  ...
  function solveT1(p) {
    ...
    var mAbs; // |m|
    var L = [];
    L.push('직선 y' + tail(-ay) + ' = m(x' + tail(-ax) + ')을 정리하면  mx − y' + (linM(k1, k0) === '0' ? '' : ' + (' + linM(k1, k0) + ')') + ' = 0');
    L.push('직선이 원에 접하려면 **원의 중심 C' + pt(cx, cy) + '에서 이 직선까지의 거리가 반지름 ' + r + '과 같아야** 하므로');
  ...
  var LINE_IDS = ['xaxis', 'yaxis', 'yx', 'ynx'];
  var T2_STORIES = [
  ...
  function solveT2(p) {
    ...
    S.push({ title: '③ 꺾인 길을 곧은 길로', lines: [
      ...
      '(두 점을 잇는 가장 짧은 길은 선분. 등호는 A′, P, Q, B′이 한 직선 위에 있을 때)'
    ] });
    var L4 = [];
    ...
    if (fixedA) {
      L5.push('점 A′에서 원 C₂′ 위의 점까지 거리의 최솟값은 (중심까지의 거리) − (반지름) 이므로');
      L5.push('▶ A′B′의 최솟값 = ' + D + ' − ' + r2 + ' = **' + ans + '**');
    } else {
      L5.push(D + ' > ' + r1 + ' + ' + r2 + ' 이므로 두 원은 서로 밖에 있고, **두 원 위의 점 사이 거리의 최솟값 = (중심 사이 거리) − r₁ − r₂**');
      L5.push('▶ A′B′의 최솟값 = ' + D + ' − ' + r1 + ' − ' + r2 + ' = **' + ans + '**');
    }
    S.push({ title: '⑤ 두 원(점과 원) 위의 점 사이의 최소 거리', lines: L5 });
    ...
    var L7 = [];
    if (orderOK) {
      L7.push('(선택 보강 — 시간이 남으면 한 줄) ' + (fixedA ? '점 A′' : '중심 C₁′') + josa(pt(c1p[0], c1p[1]), '과') + ' C₂′' + josa(pt(c2p[0], c2p[1]), '을') + ' 잇는 선분은 ' + josa(L1.name, '을') + ' P' + pt(Ppt[0], Ppt[1]) + '에서, ' + josa(L2.name, '을') + ' Q' + pt(Qpt[0], Qpt[1]) + '에서 **차례로** 지나므로 A → P → Q → B 순서의 경로가 실제로 존재한다.');
  ...
  function genT2(rng, opt) {
    ...
    for (var tries = 0; tries < 4000; tries++) {
      var L1 = opt.L1 || LINE_IDS[Math.floor(rng() * 4)];
      var L2 = opt.L2 || LINE_IDS[Math.floor(rng() * 4)];
      if (L1 === L2) continue;
  ...
  var FIXED_SETS = [
    ...
    { id: 'V7', type: 2, tag: '2번형 · x축 → y = −x, 삼조 8-15-17', p: { c1: [6, -2], r1: 1, c2: [6, 9], r2: 1, L1: 'xaxis', L2: 'ynx', story: 3, unit: null } },
  ...
  var TEMPLATES = {
    t1: [
      ...
      { h: '문제 2-1 — 기울기 m', t: ['직선 y − ▢ = m(x − ▢)를 정리하면  mx − y + (▢m + ▢) = 0', '직선이 원에 접하려면 **원의 중심 C(▢, ▢)에서 이 직선까지의 거리가 반지름 ▢과 같아야** 하므로', '|▢m + ▢| / √(m² + 1) = ▢  →  양변 제곱  →  ▢m² = ▢(m² + 1)  →  m² = ▢', '따라서 m = ▢ 또는 m = −▢  (둘 다 쓴다)'] },
    ...
    t2: [
      ...
      { h: '③ 부등식', t: ['AP + PQ + QB = A′P + PQ + QB′ ≥ **A′B′**   (등호: A′, P, Q, B′이 한 직선 위)'] },
      { h: '④ 대칭이동한 원', t: ['A′은 원 C₁을 ▢에 대칭이동한 원 C₁′: (x − ▢)² + (y − ▢)² = ▢  (중심 (▢, ▢), 반지름 ▢) 위를 움직인다.', 'B′은 원 C₂를 ▢에 대칭이동한 원 C₂′: (x − ▢)² + (y − ▢)² = ▢  (중심 (▢, ▢), 반지름 ▢) 위를 움직인다.'] },
      { h: '⑤ 두 원 위의 점 사이 최소 거리', t: ['C₁′C₂′ = √((▢ − ▢)² + (▢ − ▢)²) = √(▢ + ▢) = √▢ = ▢', '▢ > ▢ + ▢ 이므로 두 원은 서로 밖에 있고, 최솟값 = (중심 사이 거리) − r₁ − r₂ = ▢ − ▢ − ▢ = ▢'] },
  ...
  var PITFALLS = [
    ...
    { k: '2번 반지름', t: '두 원이면 **r₁ + r₂ 둘 다** 뺀다. 한쪽이 점이면(V10) 그쪽은 빼지 않는다. 중심 거리를 그대로 답으로 쓰면 오답.' },
  ...
  var QUESTIONS_FOR_SON = [
    '시험 시간은 수업시간 50분 전부인가, 아니면 그중 일부인가?',
    '1번 4소문항과 2번의 배점(15점 배분)을 선생님이 알려 주셨나?',
    '계산기·자(직선자)를 쓸 수 있나? 답안은 문제지 여백에 쓰나, 별도 답안지인가?',
    '선생님이 "숫자만 바뀐다", "대칭이동을 꼭 써라" 같은 힌트를 주셨나? 주셨다면 그대로 적어 줘.',
    '중간고사에서 원의 접선·대칭이동 문제를 맞았나, 틀렸나? 틀렸다면 어디서 막혔나(공식? 계산? 서술?).',
    '앱과 프린트에서 가장 헷갈린 부분 하나만 골라 줘. (다음 판에 반영)'
  ];
  ...
  function renderPrintPack(opts) {
    ...
    h += '<h2>이 팩의 구성과 4일 사용법</h2><table><tr><th>쪽</th><th>내용</th><th>언제</th></tr>' +
      ...
      '<tr><td>5</td><td>함정 체크리스트 · 검산 루틴 · 시험 당일 시간 배분</td><td>9/16 저녁 + 9/17 등교 전</td></tr>' +
      '<tr><td>6</td><td>아빠·클로드가 묻는 질문 6개</td><td>9/13 답을 적어 아빠에게</td></tr></table>';
    ...
    /* 1. 예시문항 + 모범답안 */
    h += '<div class="pp-page"><h2>1. 예시문항 원문과 만점 답안 예시</h2>' + problemHtml(s1, '1.') + '<h3>만점 답안 예시 — 1번</h3>' + stepsHtml(s1.steps) + '</div>';
    ...
    h += '<div class="pp-page"><h2>2. 답안 틀 카드 — ▢에 숫자만 갈아끼운다</h2><div class="pp-two"><div><h3>카드 ① 1번형</h3>' +
      ...
      '<div class="pp-note">대칭 규칙표: x축 (x, y)→(x, −y) · y축 (x, y)→(−x, y) · y = x (x, y)→(y, x) · y = −x (x, y)→(−y, −x)</div></div></div></div>';
    h += '<div class="pp-page"><h2>원리 그림 — 왜 이 방법이 되는가</h2><div class="pp-two"><div><h3>접선 = 중심에서 거리가 r인 직선</h3>' + svgTangent(SAMPLE_T1) +
      ...
      '<div class="pp-small">실선 A→P→Q→B가 실제 경로. A를 x축에, B를 y = x에 대칭이동하면(점선 원) 경로 길이는 그대로인데 모양이 직선 C₁′→C₂′(초록 점선)으로 펴진다. 두 원 위의 점 사이 최소 거리 = 중심 거리 − r₁ − r₂ = 13 − 2 − 2 = 9.</div></div></div></div>';
    /* 3. 변형 문제 쪽 */
    ...
    /* 6. 질문 */
    h += '<h2>6. 아빠·클로드가 묻는 질문 — 답을 적어 아빠에게</h2><table>' + QUESTIONS_FOR_SON.map(function (q, i) { return '<tr><td style="width:24px">' + (i + 1) + '</td><td>' + esc(q) + '<div class="pp-ans sm" style="min-height:44px;margin:4px 0 0"></div></td></tr>'; }).join('') + '</table></div>';
    h += '</div>';
    return h;
  }
  ...
  return {
    ...
    SAMPLE_T1: SAMPLE_T1, SAMPLE_T2: SAMPLE_T2, FIXED_SETS: FIXED_SETS, TEMPLATES: TEMPLATES, PITFALLS: PITFALLS, CHECKS: CHECKS, TIMEPLAN: TIMEPLAN, QUESTIONS_FOR_SON: QUESTIONS_FOR_SON,
