...
var VG = (function () {
  ...
  /* 시험 시간은 "40분 이상"(Nick 라운드 5) → 하한 40분 기준으로 배분. 계산기·자 없음, 답안은 문제지 여백. */
  var TIMEPLAN = [
  ...
  function renderPrintPack(opts) {
    ...
    h += '<h2>이 팩의 구성과 4일 사용법</h2><table><tr><th>쪽</th><th>내용</th><th>언제</th></tr>' +
      ...
      '<tr><td>3</td><td>변형 문제 10세트 (문제 쪽)</td><td>9/14·15·16 — 하루 2~3세트, 반드시 종이에 손으로. 목표 시간 1번형 16분, 2번형 15분</td></tr>' +
    ...
    h += '<div class="pp-page"><h2>3. 변형 문제 10세트 — 문제 쪽 (반드시 종이에 손으로)</h2><div class="pp-note">V1~V4 = 1번형, V5~V8 = 2번형, V9·V10 = 함정. 한 세트 목표 시간: 1번형 16분, 2번형 15분 (시험 40분 기준).</div>';
    ...
    h += '<div class="pp-page"><h2>5. 함정 체크리스트 (전날 저녁·등교 전)</h2><table class="pp-chk">' + PITFALLS.map(function (q) { return '<tr><td>☐</td><td><b>' + esc(q.k) + '</b></td><td>' + md(q.t) + '</td></tr>'; }).join('') + '</table>' +
      ...
      '<h2>시험 당일 시간 배분 (40분 기준 — 실제는 40분 이상, 계산기·자 없음, 답안은 문제지 여백)</h2><table><tr><th>구간</th><th>시간</th><th>내용</th></tr>' + TIMEPLAN.map(function (r) { return '<tr><td>' + esc(r[0]) + '</td><td>' + esc(r[1]) + '</td><td>' + esc(r[2]) + '</td></tr>'; }).join('') + '</table>';
    h += '</div>';
    h += '</div>';
    return h;
  }
