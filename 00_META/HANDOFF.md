...
> 최종 갱신: 2026-08-15 · 작업트리 클린 · 원격 동기
...
심화·시험 문항이 **기존 단원앱의 `⚡실전` 탭 안에서** 돌아간다(라운드 4 개편). 지금까지 **14세트 170문항**을 만들었고, 전량(170문항)이 **검증 게이트를 통과해 공개 배포**돼 있다 — 아이가 폰에서 링크 하나로 푼다. 한국사2·통합사회2가 완결, 통합과학2는 9단원 중 6단원 완결(Ⅳ-1~4·Ⅴ-1~2, 74문항).

지금 하는 일은 **단원 하나씩 반복**이다: 출제 → 기계 검사 → 실전 탭 이식 → 검증 게이트 → 반영 → 배포. 절차는 [`RUNBOOK.md`](../exam_track/problem_bank/RUNBOOK.md)에 고정돼 있다.

**다음 차례 = 통합과학2 `sci2_07`(Ⅴ-3 기후변화, 12문항) 검증 게이트** — 이미 작성·기계검사 통과, 게이트만 남았다(등록·스모크테스트도 남음). 그다음 Ⅵ-1·Ⅵ-2(문명·미래, 2단원)로 통합과학2를 완결한다.

⚠️ **11~13차 게이트에서 새 치명 유형 발견 — SVG 그래프 좌표 오류.** 그래프 문항(9차부터 도입)에서 좌표를 눈대중으로 그리면 서술과 어긋날 수 있다는 게 실측으로 확인됐다(2건). **그래프가 있는 문항은 좌표를 코드로 직접 검산**한다(RUNBOOK §6 게이트 프롬프트에 이미 반영, PLAN §3 참조).
...
| **통합과학2** | Ⅳ-1~Ⅳ-4·Ⅴ-1·Ⅴ-2 (6/9 완결) | 74 | ✅ | ✅ 라이브 |
| 통합과학2 | Ⅴ-3 기후변화 | 12 | 🔒 대기(작성 완료) | 차단 |
| 통합과학2 | Ⅵ-1·Ⅵ-2 (문명·미래) | — | — | — |
| 공통수학2 | 7단원 | — | — | — |
...
- `https://jwj-nick.github.io/high1/science/sci2_hub.html` (Ⅳ-1~Ⅴ-2 전부 ⚡실전 라이브)

**검증 게이트 13회 실행** — 치명 8건(복수정답 2 kh2_03 · 계산 산술오류 4 sci2_01·05·06 · **SVG 그래프 좌표 오류 2**(신규 유형) sci2_04·06)을 잡았고 나머지는 전부 진단 품질. 회차별 기록 = [`REVIEW.md`](../exam_track/problem_bank/REVIEW.md).
...
| # | 작업 | 선행 | 규모 |
|---|---|---|---|
| **1** | **통합과학2 마무리** — `sci2_07`(기후변화) 게이트 → Ⅵ-1·Ⅵ-2(문명·미래) 출제 (3단원 남음) | — | 중 |
...
│   │   └─ data/            sets.js(레지스트리+verified) · kh2_01~03 · soc2_01~05 · sci2_01~06(✅검증) · sci2_07(대기)
