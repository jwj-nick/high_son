...
심화·시험 문항이 **기존 단원앱의 `⚡실전` 탭 안에서** 돌아간다(라운드 4 개편). 지금까지 **11세트 132문항**을 만들었고, 그중 **108문항이 검증 게이트를 통과해 공개 배포**돼 있다 — 아이가 폰에서 링크 하나로 푼다.

지금 하는 일은 **단원 하나씩 반복**이다: 출제 → 기계 검사 → 실전 탭 이식 → 검증 게이트 → 반영 → 배포. 절차는 [`RUNBOOK.md`](../exam_track/problem_bank/RUNBOOK.md)에 고정돼 있다.

**다음 차례 = 통합과학2 Ⅳ-2 산·염기(`sci2_02`) 검증 게이트** — 이미 작성·기계검사 통과 상태로 대기 중, 게이트만 남았다. 그다음 `sci2_03`(지질시대) 게이트.
...
| **통합과학2** | Ⅳ-1 산화환원 | 12 | ✅ | ✅ 라이브 |
| 통합과학2 | Ⅳ-2 산염기 · Ⅳ-3 지질시대 | 24 | 🔒 대기(작성 완료) | 차단 |
| 통합과학2 | Ⅳ-4 진화 ~ Ⅵ (6단원) | — | — | — |
| 공통수학2 | 7단원 | — | — | — |
...
- `https://jwj-nick.github.io/high1/science/sci2_redox.html` (⚡실전 = `sci2_01`)

**검증 게이트 8회 실행** — 치명 3건(복수정답 2 kh2_03, 계산 산술오류 1 sci2_01)을 잡았고 나머지는 전부 진단 품질. 회차별 기록 = [`REVIEW.md`](../exam_track/problem_bank/REVIEW.md).
...
│   │   └─ data/            sets.js(레지스트리+verified) · kh2_01~03 · soc2_01~05 · sci2_01(✅검증)~03(대기)
