...
심화·시험 문항이 **기존 단원앱의 `⚡실전` 탭 안에서** 돌아간다(라운드 4 개편). 지금까지 **11세트 134문항**을 만들었고, 전량(134문항)이 **검증 게이트를 통과해 공개 배포**돼 있다 — 아이가 폰에서 링크 하나로 푼다. 한국사2·통합사회2·통합과학2 세 과목이 전부 완결됐다.

지금 하는 일은 **단원 하나씩 반복**이다: 출제 → 기계 검사 → 실전 탭 이식 → 검증 게이트 → 반영 → 배포. 절차는 [`RUNBOOK.md`](../exam_track/problem_bank/RUNBOOK.md)에 고정돼 있다.

**다음 차례 = 통합과학2 Ⅳ-4(진화) 출제**(새 단원, 게이트 대기 없음) — 또는 §5의 다른 남은 작업(세션 키트 확장·공통수학2 등). 세 과목 완결로 다음 우선순위를 정할 시점이다.
...
| **통합과학2** | Ⅳ-1·Ⅳ-2·Ⅳ-3 (3/3 완결) | 38 | ✅ | ✅ 라이브 |
| 통합과학2 | Ⅳ-4 진화 ~ Ⅵ (6단원) | — | — | — |
| 공통수학2 | 7단원 | — | — | — |
...
- `https://jwj-nick.github.io/high1/science/sci2_hub.html` (Ⅳ-1~Ⅳ-3 전부 ⚡실전 라이브)

**검증 게이트 10회 실행** — 치명 3건(복수정답 2 kh2_03, 계산 산술오류 1 sci2_01)을 잡았고 나머지는 전부 진단 품질. 회차별 기록 = [`REVIEW.md`](../exam_track/problem_bank/REVIEW.md).
...
| # | 작업 | 선행 | 규모 |
|---|---|---|---|
| **1** | **통합과학2 Ⅳ-4~Ⅵ 출제** — 진화 → 생태계 → 에너지 → 기후 → 문명 → 미래 (6단원) | — | **대** |
...
│   │   └─ data/            sets.js(레지스트리+verified) · kh2_01~03 · soc2_01~05 · sci2_01~03 (11세트 전량 ✅검증)
