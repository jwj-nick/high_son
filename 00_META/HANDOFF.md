...
> 최종 갱신: 2026-08-20-2차 (한국사 혼동쌍 탭 + 국어 2단원 앱 배포) · 작업트리 클린 · 원격 동기
...
**다음 차례(우선순위순):** ① 아이의 kh2_04 첫 풀이 결과 회수 → ingest → 재시험 루프 가동(시험 D-19). **어떤 결과가 와도 처방 자산은 전부 준비됨** — 갈래 지도가 kh2_colonial ⚡실전 탭 "📮 결과에 따라 다음 걸음" 박스에 내장(10~12→심화 / 6~9→원인코드별: C8→시기훈련·C3→혼동쌍(신규 탭)·C2→개념탭 / 0~5→정독 후 그대로 재시험 / C7·C4·C1 폴백). ingest 드라이런 실측 검증 완료(합성 결과로 파싱→일정 산정 정상) ② Nick 교과서 사진 — 속미인곡 원문 표기(옛글자 보정·본사 추가), 춘향전 수록 대목 ③ 옛글자 폰 렌더 확인 ④ 국어 3단원 별도 앱(변화하는 국어) ⑤ kh2_04 유형 전환 이월분(아이 데이터 보고 결정 — 세트 개정이라 갈래 자산 아님).
