...
| **P1 대수** | G6·G7·G8 특강 → 9단원 앱 → 허브 연결·math index·high1 index 카드 → 배포 | `alg_*` 13파일 | ✅ 2026-09-04 리뷰 2건 반영·**배포 완료**(`4107db2`) |
| **P2 미적분Ⅰ** | G9·G10·G11 특강 → 8단원 앱 → 허브·배포 | `cal1_*` 12파일 | ✅ 2026-09-04 리뷰 2건 반영·**배포 완료**(`6b75f03`) |
| **P3 확통** | G12·G13·G14 특강 → 7단원 앱 → 허브·배포 | `prob_*` 11파일 | ✅ 2026-09-04 리뷰 2건 반영·**배포 완료**(`14aa737`) |
...
## 9. 후속 후보 · 세부 계획 (2026-09-05)

3과목 배포 뒤 남은 후보를 전부 나열하고, 외부 의존이 없는 것은 같은 날 자율 진행으로 끝냈다.

| # | 후보 | 대상 파일 | 결정 · 결과 |
|---|---|---|---|
| 1 | 퀴즈 자기완결 루프 (FLOW V2 미니 이식) | `대수/10_app/alg_app.js` (24앱 공유) | ✅ 완료. [즉시] 🩹 고치기 + "🎲 사실 찍었어"(찍맞 = 모름) → [라운드] 틀린·찍은 문항 🔁 되풀이(최대 3회전, 사다리 불변) → [세션] 📬 복습함 `localStorage quizbox:<앱id>`(due=내일, 1·3·7일 사다리, 3연속 정답 = 🏅 정복, due 전 정답은 무변화). 퀴즈를 열 때 때가 된 문항이 있으면 "복습부터 풀기" 배너. 헤드리스 시나리오 시험 33건 통과. drill.js의 원인 코드(C1~C8)·확신도 4단계는 넣지 않았다 — 객관식 10문·🩹 탭 매핑이 진단을 대신하고, 확신도는 "찍었어" 한 단계로 축약(마찰 최소). |
| 2 | `alg_explog` 인라인 엔진 → 공용 엔진 이관 | `대수/10_app/alg_explog.html` | ✅ 완료. 첫 앱이라 자체 퀴즈·탭 코드를 갖고 있었음(루프 미적용 사각지대). `APP.onTab`·`APP.quiz`·`stepbtn data-w`로 통일. |
| 3 | 3부작 링크 활성화 | `cal1_sp_slope.html` · `prob_sp_choose.html` | ✅ 완료. 중2 특강(`mid2/slope-mastery.html`·`mid2/counting-mastery.html`)이 2026-09-04 math-story-telling에서 배포되어 "준비 중" 칩을 실제 링크로 교체. 세기 3부작은 고1 `mat1_counting.html`도 연결. |
| 4 | 허브 상태 배지·진행바 | 허브 3 | ✅ 완료. 리뷰·배포가 끝난 카드 전부 `신규`→`완성`(진행 100%). 진행 라벨은 0인 항목을 생략. |
| 5 | 슬라이더 터치 타겟 44px | `_shared/lecture.js`(특강 38) · 과목 CSS 3 · `alg_explog` | ✅ 완료. `input[type=range]`에 `min-height:44px`. |
| 6 | 평균값 정리 슬라이더 끝값 스냅 | `cal1_tangent.html` | ✅ 완료. 슬라이더 범위에서 구간 양 끝을 제외해, 끝으로 끌었을 때 가운데로 튀지 않는다. |
| 7 | 퀴즈 HUD "점수 n/10" · 완료 화면 "정답률" 문구 | 단원 앱 24 + 고1 앱(갈래 전체 55파일) | ⏸ **Nick 판단.** 금지사항 1("점수")과 문구 충돌 소지가 있으나 게이미피케이션 성격이고 갈래 전체 관행이라 유지. 바꾸려면 "맞힌 수 n/10"·"n문항 중 m문항"으로 전 과목 일괄 교체(마크업 24 + 엔진 1 + 고1 인라인 30여). |
| 8 | `.industry`(🏭) 스타일을 lecture.js에 추가 | — | ✗ 불필요. 특강 페이지는 🏭 블록을 쓰지 않는다(단원 앱만 인라인 정의). 메모의 오기. |
| 9 | 아들 사용 피드백 반영 · 교과서 확보 시 용어·순서 보정 | 과목별 `10_app/` | ⏳ 외부 의존. 피드백이 오면 §7 게이트 그대로. |
| 10 | 전 단원 복습함 섞어 풀기(시험 직전 모드) | — | ✗ 내신 갈래 소관(FLOW V2 §5-5). 이 갈래에서는 만들지 않는다. |
| 11 | 고1 공통수학1·2 앱 13에 같은 루프 이식 | `공통수학1/10_app/mat1_*` 6 · `공통수학2/10_app/mat2_*` 7 · 허브 2 | ✅ 완료(같은 날 2차). 인라인 엔진 형태가 13개 전부 동일해 스크립트(`migrate_mat.js`)로 이관: `go`·탭 리스너·퀴즈 블록 삭제 → `alg_app.js` + `APP.onTab({탭:훅})` + `APP.quiz(QUIZ, null, {perfect})`, 초기화 호출(`selD`·`calc`·`drawNL`)은 보존, QUIZ `<x`→`&lt;x`(innerHTML 전환). 엔진에 `.fix` 기본 스타일 추가. 배포 5d98a34. **엔진 수정(고2 24앱에도 적용):** 해시 딥링크(#탭)를 setTimeout(0)으로 열면 외부 스크립트 뒤 파서 양보 시 인라인 APP.onTab 등록보다 먼저 실행되어 캔버스 훅이 빠지는 경합을 실측(이차함수 #graph 빈 캔버스) → DOMContentLoaded 이후로 이동, 5회 반복 렌더 확인. 후속 후보: QUIZ에 `t`(고치는 곳 탭) 붙여 🩹 칩 활성화. |
| 12 | 왼쪽 사이드바 · 수학 전체 허브 · 학기 디렉토리 | `high1/math/` 전체 · 앱 56 | ✅ **P1·P2 완료·배포 `cfb863f`**(2026-09-06) · P3만 남음. 중등에 있는 왼쪽 사이드바가 고등에는 없고, `high1/math/`가 학기 구분 없이 평면이다. 목표·단계·위험 = [`수학허브_개편_BACKLOG.md`](수학허브_개편_BACKLOG.md). P1 사이드바·P2 허브는 지금도 착수 가능, **P3 디렉토리 이동은 점검 세트 배치 1·2가 끝난 뒤**(파일 이동과 세트 이식이 충돌). |

**게이트(후속 공통):** `node --check` → 헤드리스 시나리오 시험(`scratchpad/render/qtest.html`, 엔진 변경 시) → `app_check`/`lecture_check` → 스크린샷 → `se-agent-app-reviewer`(sonnet) → 파일 단위 배포 → curl 200.

## 변경 이력
| 날짜 | 내용 |
|---|---|
| 2026-09-07 | **대수 9단원에 ⚡ 실전 점검 세트 108문항 완비**(게이트 25~29차, 배포 3ef3f7c·bc73d9f·fa0c17e·232f770). 앱 개념 박스 20개 보강. 정본은 `exam_track/problem_bank/data/alg_01~09.js`, 계획은 `MATH_DRILL_PLAN.md` §1 배치 3. 다음은 미적분Ⅰ 8앱. |
...
| 2026-09-05 | **후속 10건 정리·자율 진행.** §9 신설. 공용 퀴즈 엔진 v2(🔁 되풀이·📬 복습함·🎲 찍었어, 시나리오 시험 33건) + `alg_explog` 공용 엔진 이관 + 3부작 링크 + 허브 완성 배지 + 슬라이더 44px + MVT 슬라이더. 리뷰어(sonnet) 1건 반영. 배포 커밋 042c333. |
| 2026-09-05 (2차) | **고1 공통수학1·2 앱 13 공용 엔진 이관**(§9 행 11). 배포 5d98a34. |
| 2026-09-06 | §9 행 12 추가 — 수학 허브 개편 백로그(사이드바·전체 허브·학기 디렉토리) 등재. 정본 = `수학허브_개편_BACKLOG.md`. |
