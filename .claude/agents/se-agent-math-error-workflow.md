---
name: se-agent-math-error-workflow
description: 수학 오답노트 전체 워크플로우를 자동화하는 에이전트. 틀린 문제 번호 목록을 받아 se-math-error-note → se-math-figure(그림 있는 경우) → se-math-practice 스킬을 순서대로 실행하고, app-reviewer로 검증 후 todo.md를 업데이트한다. 호출 예시 — "수학 오답 Q12, Q13, Q15 처리해줘", "오답노트 에이전트 실행".
tools: [Read, Write, Edit, Bash, Glob, Grep]
---

# se-agent-math-error-workflow 에이전트
...
- se-math-error-note 또는 se-math-practice 스킬을 여러 문제에 일괄 적용할 때
...
3. se-math-error-note 스킬 실행 → Q<N>_<주제>.md + Q<N>_app.html 생성
4. [그림 있는 경우] se-math-figure 스킬 실행 → 문제 페이지: 정적 SVG / 풀이 페이지: JSXGraph
5. se-math-practice 스킬 실행 → Q<N>_practice.md + Q<N>_practice_app.html 생성
6. se-agent-app-reviewer 에이전트 호출 → APP_PRINCIPLES 기준 검증, 위반 즉시 수정
...
→ 위 중 하나라도 해당하면 se-math-figure 스킬 실행
...
- se-agent-app-reviewer 검증에서 위반 발견 시: 단순 텍스트 → 즉시 수정 / 구조적 → Nick 보고
